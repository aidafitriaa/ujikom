...
<div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <center>
                        <div class="card-header">Show Kredit</div>
                    </center>
    
                    <div class="card-body">
                        <form action="{{route('kredit.show', $kredit->id)}}" method="POST">
                            <input type="hidden" name="_method" value="PATCH">
                            @csrf
                            <div class="form-group">
                                <label for="">Kode Kredit</label>
                                <input class="form-control" value="{{$kredit->kode_kredit}}" type="text" name="kode_kredit"disabled>
                            </div>
                            <div class="form-group">
                                <label for="">KTP</label>
                                <input class="form-control" value="{{$kredit->KTP}}"type="text" name="KTP"disabled>
                            </div>
                            <div class="form-group">
                                <label for="">Id Paket</label>
                                <input class="form-control" value="{{$kredit->paket_kredits_id}}"type="text" name="paket_kredits_id"disabled>
                            </div>
                            <div class="form-group">
                                <label for="">Id Mobil</label>
                                <input class="form-control" value="{{$kredit->mobils_id}}"type="text" name="mobils_id"disabled>
                            </div>
                            <div class="form-group">
                                <label for="">Tanggal Kredit</label>
                                <input class="form-control" value="{{$kredit->tgl_kredit}}"type="text" name="tgl_kredit"disabled>
                            </div>
                            <div>
                                    <div class="form-group">
                                        <a href="{{url('/kredit')}}" class="btn btn-outline-info">Kembali</a>
                                    </div>
                            </div>
                        </form>
                    </div>
